...
export const SessionItem = ({ session }: { session: Session }) => {
  ...
  return (
    <>
      <div className="h-full cursor-pointer bg-white/10 p-6 text-left" onClick={() => setShowSessionDetail(true)}>
...
function SessionDetailPopup({ session, close }: { session: Session; close: () => void }) {
